...
#include "mem/ruby/structures/RubyBingoPrefetcher.hh"
...
public:
  Table(int width, int height)
      : width(width), height(height), cells(height, vector<string>(width)) {}
...
public:
  ...
  SetAssociativeCache(int size, int num_ways, int debug_level = 0)
      : size(size), num_ways(num_ways), num_sets(size / num_ways),
        entries(num_sets, vector<Entry>(num_ways)), cams(num_sets),
  ...
  /**
   * Creates a table with the given headers and populates the rows by calling
   * `write_data` on all valid entries contained in the cache. This function
   * makes it easy to visualize the contents of a cache.
...
public:
  LRUSetAssociativeCache(int size, int num_ways, int debug_level = 0)
      : Super(size, num_ways, debug_level),
        lru(this->num_sets, vector<uint64_t>(num_ways)) {}
 ...
 * 1) Splits key into blocks of length `index_len` bits and computes the XOR of
 * all blocks. 2) Replaces the least significant block of key with computed
 * block. With this hash function, the index will depend on all bits in the key.
 * As a consequence, entries will be more randomly distributed among the sets.
 * NOTE: Applying this hash function twice with the same `index_len` acts as the
 * identity function.
 */
...
public:
  FilterTable(int size, int debug_level = 0, int num_ways = 16)
    ...
    if (this->debug_level >= 1)
      cerr << "FilterTable::FilterTable(size=" << size
  ...
  Entry *find(uint64_t region_number) {
    if (this->debug_level >= 2)
      cerr << "FilterTable::find(region_number=0x" << hex << region_number
           << ")" << dec << endl;
...
public:
  AccumulationTable(int size, int pattern_len, int debug_level = 0,
                    int num_ways = 16)
      : Super(size, num_ways, debug_level), pattern_len(pattern_len) {
    // assert(__builtin_popcount(size) == 1);
    // assert(__builtin_popcount(pattern_len) == 1);
    if (this->debug_level >= 1)
      cerr << "AccumulationTable::AccumulationTable(size=" << size
           << ", pattern_len=" << pattern_len << ", debug_level=" << debug_level
  ...
  bool set_pattern(uint64_t region_number, int offset) {
    if (this->debug_level >= 2)
      cerr << "AccumulationTable::set_pattern(region_number=0x" << hex
           << region_number << ", offset=" << dec << offset << ")" << dec
           << endl;
    uint64_t key = this->build_key(region_number);
    Entry *entry = Super::find(key);
    if (!entry) {
  ...
  Entry insert(uint64_t region_number, uint64_t pc, int offset) {
    if (this->debug_level >= 2)
      cerr << "AccumulationTable::insert(region_number=0x" << hex
           << region_number << ", pc=0x" << pc << ", offset=" << dec << offset
           << dec << endl;
    uint64_t key = this->build_key(region_number);
    // assert(!Super::find(key));
...
class PatternHistoryTable
    : public LRUSetAssociativeCache<PatternHistoryTableData> {
  typedef LRUSetAssociativeCache<PatternHistoryTableData> Super;

public:
  PatternHistoryTable(int size, int pattern_len, int min_addr_width,
                      int max_addr_width, int pc_width, int debug_level = 0,
                      int num_ways = 16)
      : Super(size, num_ways, debug_level), pattern_len(pattern_len),
    ...
    // assert(__builtin_popcount(pattern_len) == 1);
    if (this->debug_level >= 1)
      cerr << "PatternHistoryTable::PatternHistoryTable(size=" << size
           << ", pattern_len=" << pattern_len
           << ", min_addr_width=" << min_addr_width
  ...
  void insert(uint64_t pc, uint64_t address, vector<bool> pattern) {
    if (this->debug_level >= 2)
      cerr << "PatternHistoryTable::insert(pc=0x" << hex << pc << ", address=0x"
           << address << ", pattern=" << pattern_to_string(pattern) << ")"
           << dec << endl;
  ...
  vector<vector<bool>> find(uint64_t pc, uint64_t address) {
    if (this->debug_level >= 2)
      cerr << "PatternHistoryTable::find(pc=0x" << hex << pc << ", address=0x"
           << address << ")" << dec << endl;
    uint64_t key = this->build_key(pc, address);
    uint64_t index = key % this->num_sets;
    uint64_t tag = key / this->num_sets;
    auto &set = this->entries[index];
    uint64_t min_tag_mask =
        (1 << (this->pc_width + this->min_addr_width - this->index_len)) - 1;
    uint64_t max_tag_mask =
        (1 << (this->pc_width + this->max_addr_width - this->index_len)) - 1;
...
private:
  ...
  void write_data(Entry &entry, Table &table, int row) {
    ...
    uint64_t index_key =
        entry.key & ((1 << (this->pc_width + this->min_addr_width)) - 1);
    index_key = hash_index(index_key, this->index_len); /* unhash */
    uint64_t key =
        (base_key << (this->pc_width + this->min_addr_width)) | index_key;
  ...
  uint64_t build_key(uint64_t pc, uint64_t address) {
    ...
    address &= (1 << this->max_addr_width) -
               1; /* use `addr_width` bits from address */
    ...
    uint64_t index_key =
        hash_index((pc << this->min_addr_width) | offset, this->index_len);
    uint64_t key =
        (base << (this->pc_width + this->min_addr_width)) | index_key;
    return key;
  }
...
public:
  ...
  int prefetch(RubyBingoPrefetcher *pf, uint64_t block_address) {
    if (this->debug_level >= 2) {
      ...
      //        << endl;
      //   cerr << "[PrefetchStreamer::prefetch] " << cache->PQ.occupancy << "/"
      //        << cache->PQ.SIZE << " PQ entries occupied." << dec << endl;
      //   cerr << "[PrefetchStreamer::prefetch] " << cache->MSHR.occupancy <<
      //   "/"
...
class Bingo {
public:
  Bingo(int pattern_len, int min_addr_width, int max_addr_width, int pc_width,
        int filter_table_size, int accumulation_table_size, int pht_size,
        int pht_ways, int pf_streamer_size, int debug_level = 0)
      : pattern_len(pattern_len), filter_table(filter_table_size, debug_level),
        ...
        pht(pht_size, pattern_len, min_addr_width, max_addr_width, pc_width,
            debug_level, pht_ways),
        pf_streamer(pf_streamer_size, pattern_len, debug_level),
        debug_level(debug_level) {
    if (this->debug_level >= 1)
      cerr << "Bingo::Bingo(pattern_len=" << pattern_len
           << ", min_addr_width=" << min_addr_width
           << ", max_addr_width=" << max_addr_width << ", pc_width=" << pc_width
           << ", filter_table_size=" << filter_table_size
           << ", accumulation_table_size=" << accumulation_table_size
           << ", pht_size=" << pht_size << ", pht_ways=" << pht_ways
           << ", pf_streamer_size=" << pf_streamer_size
  ...
  void access(uint64_t block_number, uint64_t pc) {
    if (this->debug_level >= 2)
      cerr << "[Bingo] access(block_number=0x" << hex << block_number
           << ", pc=0x" << pc << ")" << dec << endl;
    ...
    if (entry->data.offset != region_offset) {
      ...
      uint64_t region_number =
          hash_index(entry->key, this->filter_table.get_index_len());
      AccumulationTable::Entry victim = this->accumulation_table.insert(
          region_number, entry->data.pc, entry->data.offset);
      this->accumulation_table.set_pattern(region_number, region_offset);
  ...
  void eviction(uint64_t block_number) {
    if (this->debug_level >= 2)
      cerr << "[Bingo] eviction(block_number=" << block_number << ")" << dec
           << endl;
    /* end of generation: footprint must now be stored in PHT */
    uint64_t region_number = block_number / this->pattern_len;
    this->filter_table.erase(region_number);
    AccumulationTable::Entry *entry =
        this->accumulation_table.erase(region_number);
    if (entry) {
      /* move from accumulation table to PHT */
  ...
  int prefetch(RubyBingoPrefetcher *pf, uint64_t block_number) {
    ...
    //        endl;
    int pf_issued = this->pf_streamer.prefetch(pf, block_number);
    // if (this->debug_level >= 2)
  ...
  void print_stats() {
    ...
    cout << "[Bingo] Prefetch PC+Addr: " << this->prefetch_cnt[PC_ADDRESS]
         << endl;
    cout << "[Bingo] Prefetch PC+Offs: " << this->prefetch_cnt[PC_OFFSET]
         << endl;

    cout << "[Bingo] Useful PC+Addr: " << this->useful_cnt[PC_ADDRESS] << endl;
    cout << "[Bingo] Useful PC+Offs: " << this->useful_cnt[PC_OFFSET] << endl;

    cout << "[Bingo] Useless PC+Addr: " << this->useless_cnt[PC_ADDRESS]
         << endl;
    cout << "[Bingo] Useless PC+Offs: " << this->useless_cnt[PC_OFFSET] << endl;

    double l1_pref_per_region =
        1.0 * this->pref_level_cnt[FILL_L1] / this->region_pref_cnt;
    double l2_pref_per_region =
        1.0 * this->pref_level_cnt[FILL_L2] / this->region_pref_cnt;
    double l3_pref_per_region =
        1.0 * this->pref_level_cnt[FILL_LLC] / this->region_pref_cnt;
    double no_pref_per_region =
        (double)this->pattern_len -
...
private:
  ...
  vector<int> find_in_pht(uint64_t pc, uint64_t address) {
    if (this->debug_level >= 2) {
      cerr << "[Bingo] find_in_pht(pc=0x" << hex << pc << ", address=0x"
           << address << ")" << dec << endl;
    }
  ...
  vector<int> vote(const vector<vector<bool>> &x) {
    ...
    if (this->debug_level >= 2) {
      ...
      for (int i = 0; i < n; i += 1)
        cerr << "<" << setw(3) << i + 1 << "> " << pattern_to_string(x[i])
             << endl;
    }
    bool pf_flag = false;
...
Addr RubyBingoPrefetcher::pageAddress(Addr addr) const {
  return mbits<Addr>(addr, 63, m_page_shift);
}

void RubyBingoPrefetcher::regStats() {
  SimObject::regStats();
  ...
  numPrefetchRequested.name(name() + ".prefetches_requested")
      .desc("number of prefetch requests made");

  numHits.name(name() + ".hits")
      .desc("number of prefetched blocks accessed (for the first time)");

  numPartialHits.name(name() + ".partial_hits")
      .desc("number of misses observed for a block being prefetched");

  numPagesCrossed.name(name() + ".pages_crossed")
      .desc("number of prefetches across pages");

  numMissedPrefetchedBlocks.name(name() + ".misses_on_prefetched_blocks")
...
void RubyBingoPrefetcher::observeReq(Addr address, Addr pc, bool hit,
  ...
  /* issue prefetches */
  this->bingo->prefetch(this, blockNumber);
}
...
void RubyBingoPrefetcher::observeEvict(Addr evictedAddr) {
  ...
  this->bingo->eviction(evictedBlockNumber);
}

int RubyBingoPrefetcher::prefetchLine(Addr pc, Addr baseAddr, Addr pfAddr,
                                      int pfFillLevel) {
