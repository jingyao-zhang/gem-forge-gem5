...
StreamAwareCache::~StreamAwareCache() {
...
#undef SUM_NON_DEMAND
  ...
  this->numCoalescedStreamPredictedMissWrong
      .name(this->name() + ".numCoalescedStreamPredictedMissWrong")
      .desc("Number of times a request from coalesced stream is predicted to "
            "be miss but hit.")
      .prereq(this->numCoalescedStreamPredictedMissWrong);

  this->numCoalescedStreamPredictedMiss
      .name(this->name() + ".numCoalescedStreamPredictedMiss")
      .desc("Number of times a request from coalesced stream is predicted to "
            "be miss.")
      .prereq(this->numCoalescedStreamPredictedMiss);
}
...
void StreamAwareCache::satisfyRequest(PacketPtr pkt, CacheBlk *blk,
      ...
      if (pkt->needsWritable()) {
        ...
        if (blk->isDirty()) {
          ...
          blk->status &= ~BlkDirty;
                 ...
                 !pkt->hasSharers() && pkt->cmd != MemCmd::ReadCleanReq) {
...
bool StreamAwareCache::access(PacketPtr pkt, CacheBlk *&blk, Cycles &lat,
...
void StreamAwareCache::maintainClusivity(bool from_cache, CacheBlk *blk) {
...
void StreamAwareCache::doWritebacks(PacketList &writebacks, Tick forward_time) {
...
void StreamAwareCache::doWritebacksAtomic(PacketList &writebacks) {
  while (!writebacks.empty()) {
    PacketPtr wbPkt = writebacks.front();
...
void StreamAwareCache::recvTimingSnoopResp(PacketPtr pkt) {
...
void StreamAwareCache::promoteWholeLineWrites(PacketPtr pkt) {
...
bool StreamAwareCache::recvTimingReq(PacketPtr pkt) {

  if (this->shouldUseStreamAwareMissPredictionPolicy(pkt)) {
    return this->recvTimingReqForStream(pkt);
  }
  ...
  if (satisfied) {
    ...
    // uncacheable request
    MSHR *mshr = pkt->req->isUncacheable()
                     ? nullptr
...
PacketPtr StreamAwareCache::createMissPacket(PacketPtr cpu_pkt, CacheBlk *blk,
...
Tick StreamAwareCache::recvAtomic(PacketPtr pkt) {
    ...
    if (!is_forward) {
      if (pkt->needsResponse()) {
        ...
        if (bus_pkt->isError()) {
          ...
          blk = handleFill(pkt, blk, writebacks, allocOnFill(pkt->cmd));
          assert(blk != NULL);
          is_invalidate = false;
          satisfyRequest(pkt, blk);
        } else if (bus_pkt->isRead() || bus_pkt->cmd == MemCmd::UpgradeResp) {
          // we're updating cache state to allow us to
          // satisfy the upstream request from the cache
          blk = handleFill(bus_pkt, blk, writebacks, allocOnFill(pkt->cmd));
          satisfyRequest(pkt, blk);
          maintainClusivity(pkt->fromCache(), blk);
        } else {
...
void StreamAwareCache::functionalAccess(PacketPtr pkt, bool fromCpuSide) {
  if (system->bypassCaches()) {
  ...
  bool have_data =
      blk && blk->isValid() &&
      pkt->checkFunctional(&cbpw, blk_addr, is_secure, blkSize, blk->data);
  ...
  bool done = have_dirty || cpuSidePort->checkFunctional(pkt) ||
...
void StreamAwareCache::handleUncacheableWriteResp(PacketPtr pkt) {
...
void StreamAwareCache::recvTimingResp(PacketPtr pkt) {
  assert(pkt->isResponse());
  ...
  for (auto &target : targets) {
    ...
    case MSHR::Target::FromCPU:
      ...
      if (tgt_pkt->cmd == MemCmd::WriteLineReq) {
        ...
        blk = handleFill(tgt_pkt, blk, writebacks, targets.allocOnFill);
        assert(blk != nullptr);
      ...
      if (is_fill) {
        ...
        int transfer_offset = tgt_pkt->getOffset(blkSize) - initial_offset;
...
PacketPtr StreamAwareCache::writebackBlk(CacheBlk *blk) {
...
PacketPtr StreamAwareCache::cleanEvictBlk(CacheBlk *blk) {
  assert(!writebackClean);
...
void StreamAwareCache::memWriteback() {
  StreamAwareCacheBlkVisitorWrapper visitor(
      *this, &StreamAwareCache::writebackVisitor);
  tags->forEachBlk(visitor);
}

void StreamAwareCache::memInvalidate() {
  StreamAwareCacheBlkVisitorWrapper visitor(
      *this, &StreamAwareCache::invalidateVisitor);
  tags->forEachBlk(visitor);
}

bool StreamAwareCache::isDirty() const {
  StreamAwareCacheBlkIsDirtyVisitor visitor;
  tags->forEachBlk(visitor);

  return visitor.isDirty();
}

bool StreamAwareCache::writebackVisitor(CacheBlk &blk) {
  if (blk.isDirty()) {
    assert(blk.isValid());

    Request request(tags->regenerateBlkAddr(blk.tag, blk.set), blkSize, 0,
...
bool StreamAwareCache::invalidateVisitor(CacheBlk &blk) {
...
CacheBlk *StreamAwareCache::allocateBlock(Addr addr, bool is_secure,
                                          PacketList &writebacks) {
  ...
  if (blk->isValid()) {
    ...
    if (repl_mshr) {
      ...
      DPRINTF(Cache,
              ...
              "(%s): %s\n",
              repl_addr, blk->isSecure() ? "s" : "ns", addr,
              is_secure ? "s" : "ns", blk->isDirty() ? "writeback" : "clean");
...
void StreamAwareCache::invalidateBlock(CacheBlk *blk) {
  if (blk != tempBlock)
    tags->invalidate(blk);
  blk->invalidate();
}
...
CacheBlk *StreamAwareCache::handleFill(PacketPtr pkt, CacheBlk *blk,
  ...
  blk->whenReady =
      clockEdge() + fillLatency * clockPeriod() + pkt->payloadDelay;
...
void StreamAwareCache::doTimingSupplyResponse(PacketPtr req_pkt,
...
uint32_t StreamAwareCache::handleSnoop(PacketPtr pkt, CacheBlk *blk,
...
void StreamAwareCache::recvTimingSnoopReq(PacketPtr pkt) {
  DPRINTF(CacheVerbose, "%s: for %s\n", __func__, pkt->print());
  ...
  if (wb_entry) {
    ...
    if (respond) {
      ...
      doTimingSupplyResponse(pkt, wb_pkt->getConstPtr<uint8_t>(), false, false);
    }
...
bool StreamAwareCache::CpuSidePort::recvTimingSnoopResp(PacketPtr pkt) {
  // Express snoop responses from master to slave, e.g., from L1 to L2
  cache->recvTimingSnoopResp(pkt);
  return true;
}

Tick StreamAwareCache::recvAtomicSnoop(PacketPtr pkt) {
  // Snoops shouldn't happen when bypassing caches
  assert(!system->bypassCaches());
...
QueueEntry *StreamAwareCache::getNextQueueEntry() {
  ...
  if (wq_entry && (writeBuffer.isFull() || !miss_mshr)) {
    ...
    MSHR *conflict_mshr =
        mshrQueue.findPending(wq_entry->blkAddr, wq_entry->isSecure);
    ...
    WriteQueueEntry *conflict_mshr =
        writeBuffer.findPending(miss_mshr->blkAddr, miss_mshr->isSecure);
    if (conflict_mshr) {
...
bool StreamAwareCache::isCachedAbove(PacketPtr pkt, bool is_timing) const {
  if (!forwardSnoops)
    return false;
...
Tick StreamAwareCache::nextQueueReadyTime() const {
  Tick nextReady =
...
bool StreamAwareCache::sendMSHRQueuePacket(MSHR *mshr) {
  assert(mshr);
...
bool StreamAwareCache::sendWriteQueuePacket(WriteQueueEntry *wq_entry) {
  assert(wq_entry);
...
void StreamAwareCache::serialize(CheckpointOut &cp) const {
  bool dirty(isDirty());
...
void StreamAwareCache::unserialize(CheckpointIn &cp) {
  bool bad_checkpoint;
...
AddrRangeList StreamAwareCache::CpuSidePort::getAddrRanges() const {
  return cache->getAddrRanges();
}

bool StreamAwareCache::CpuSidePort::recvTimingReq(PacketPtr pkt) {
...
Tick StreamAwareCache::CpuSidePort::recvAtomic(PacketPtr pkt) {
  return cache->recvAtomic(pkt);
}

void StreamAwareCache::CpuSidePort::recvFunctional(PacketPtr pkt) {
  // functional request
  cache->functionalAccess(pkt, true);
}

StreamAwareCache::CpuSidePort::CpuSidePort(const std::string &_name,
                                           StreamAwareCache *_cache,
                                           const std::string &_label)
    : BaseCache::CacheSlavePort(_name, _cache, _label), cache(_cache) {}

StreamAwareCache *StreamAwareCacheParams::create() {
  assert(tags);

  return new StreamAwareCache(this);
}
///////////////
//
// MemSidePort
...
bool StreamAwareCache::MemSidePort::recvTimingResp(PacketPtr pkt) {
  cache->recvTimingResp(pkt);
  return true;
}

// Express snooping requests to memside port
void StreamAwareCache::MemSidePort::recvTimingSnoopReq(PacketPtr pkt) {
  // handle snooping requests
  cache->recvTimingSnoopReq(pkt);
}

Tick StreamAwareCache::MemSidePort::recvAtomicSnoop(PacketPtr pkt) {
  return cache->recvAtomicSnoop(pkt);
}

void StreamAwareCache::MemSidePort::recvFunctionalSnoop(PacketPtr pkt) {
...
void StreamAwareCache::CacheReqPacketQueue::sendDeferredPacket() {
  // sanity check
...
StreamAwareCache::MemSidePort::MemSidePort(const std::string &_name,
                                           StreamAwareCache *_cache,
                                           const std::string &_label)
...
bool StreamAwareCache::recvTimingReqForStream(PacketPtr pkt) {
  DPRINTF(CacheTags, "%s tags:\n%s\n", __func__, tags->print());

  assert(pkt->isRequest());

  bool predictedMiss = this->predictCoalescedStreamRequest(pkt);
