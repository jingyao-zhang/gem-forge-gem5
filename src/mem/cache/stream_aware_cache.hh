...
#ifndef __MEM_CACHE_STREAM_AWARE_CACHE_HH__
#define __MEM_CACHE_STREAM_AWARE_CACHE_HH__
...
/**
 * StreamAwareCache. So far only can be used as L1 cache.
 */
class StreamAwareCache : public BaseCache {
protected:
  /**
  ...
  public:
    CpuSidePort(const std::string &_name, StreamAwareCache *_cache,
  ...
  public:
    MemSidePort(const std::string &_name, StreamAwareCache *_cache,
                const std::string &_label);
  };
  ...
  inline bool allocOnFill(MemCmd cmd) const override {
    return clusivity == Enums::mostly_incl || cmd == MemCmd::WriteLineReq ||
           cmd == MemCmd::ReadReq || cmd == MemCmd::WriteReq ||
           cmd.isPrefetch() || cmd.isLLSC();
  }
   ...
   */
  bool outstandingMisses() const { return !mshrQueue.isEmpty(); }
...
public:
  ...
  StreamAwareCache(const StreamAwareCacheParams *p);

  /** Non-default destructor is needed to deallocate memory. */
  virtual ~StreamAwareCache();
 ...
 */
class StreamAwareCacheBlkVisitorWrapper : public CacheBlkVisitor {
public:
  typedef bool (StreamAwareCache::*VisitorPtr)(CacheBlk &blk);

  StreamAwareCacheBlkVisitorWrapper(StreamAwareCache &_cache,
                                    VisitorPtr _visitor)
      : cache(_cache), visitor(_visitor) {}

  bool operator()(CacheBlk &blk) override { return (cache.*visitor)(blk); }
 ...
 */
class StreamAwareCacheBlkIsDirtyVisitor : public CacheBlkVisitor {
public:
  StreamAwareCacheBlkIsDirtyVisitor() : _isDirty(false) {}
