...
public:
  ...
  PUMCommandVecT analyze_stream_pair(const AffinePattern &src_stream,
    ...
    auto no_reuse_src_sub_region =
        fix_reuse_in_canonical_sub_region(src_stream);
  ...
  PUMCommandVecT handle_align_smaller_than_tile_size(int64_t dim,
    ...
    front_starts.push_back(0);
    front_trips.push_back(distance);
    ...
    back_starts.push_back(tile_sizes[dim] - distance);
    back_trips.push_back(distance);
    for (auto i = dim + 1; i < dimension; ++i) {
      back_starts.push_back(0);
      back_trips.push_back(tile_sizes[i]);
    }
