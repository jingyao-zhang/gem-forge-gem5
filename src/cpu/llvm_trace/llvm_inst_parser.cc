#include "llvm_inst_parser.hh"

// For the DPRINTF function.
#include "base/trace.hh"
#include "debug/LLVMTraceCPU.hh"

LLVMInstParser::LLVMInstParser(const std::string &fileName) {
  this->input = new ProtoInputStream(fileName);
}

size_t
LLVMInstParser::parse(std::list<std::shared_ptr<LLVMDynamicInst>> &buffer) {
  size_t count = 0;

  if (this->input->read(this->TDG)) {
    for (size_t i = 0; i < this->TDG.instructions_size(); ++i) {
      auto &inst = this->TDG.instructions(i);
 is sent
      std::shared_ptr<LLVMDynamicInst> llvmInst;
      ...
      case LLVM::TDG::TDGInstruction::ExtraCase::kStore: {
        ...
        llvmInst = std::make_shared<LLVMDynamicInstMem>(
      ...
      case LLVM::TDG::TDGInstruction::ExtraCase::kBranch: {
        ...
        llvmInst = std::make_shared<LLVMDynamicInstCompute>(
            inst.id(), inst.op(), numMicroOps, std::move(dependentInstIds),
      ...
      buffer.emplace_back(std::move(llvmInst));
      count++;
    }
