...
class LLVMBranchPredictor {
 public:
  ...
  virtual bool predictAndUpdate(const std::shared_ptr<LLVMDynamicInst>& inst);

 protected:
  std::unordered_map<uint64_t, std::array<std::string, 2>> records;

  bool predict(const std::shared_ptr<LLVMDynamicInst>& inst) const;
  void update(const std::shared_ptr<LLVMDynamicInst>& inst);
};
